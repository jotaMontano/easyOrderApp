...
import com.moonshine.service.CategoryService;
...
public class CategoryResource {
    ...
    @GetMapping("/categories/getCategoriesByClient/{id}")
    @Timed
    public ResponseEntity<List<CategoryDTO>> getProductsByClient(@PathVariable Long id, Pageable pageable) {
        Page<CategoryDTO> page = categoryService.getCategoriesByClient(id, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/getCategoriesByClient/{id}");
        return ResponseEntity.ok().headers(headers).body(page.getContent());

    }

    @GetMapping("/categoriesByClient/{id}")
    public ResponseEntity<List<CategoryDTO>> getAllCategories(@PathVariable Long id, Pageable pageable) {
        log.debug("REST request to get a page of Extras");
        Page<CategoryDTO> page = categoryService.findAllByClient(id,pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/categories");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    @GetMapping("/categories/getCategoriesByStatus/{id}/{status}")
    @Timed
    public ResponseEntity<List<CategoryDTO>> getDiscountsByStatus(@PathVariable Long id,  @PathVariable boolean status, Pageable pageable) {
        Page<CategoryDTO> page = categoryService.getCategoriesBystatus(id, status, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/getCategoriesByStatus/{id}/{status}");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
