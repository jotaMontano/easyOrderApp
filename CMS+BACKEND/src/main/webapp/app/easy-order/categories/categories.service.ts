...
export class CategoriesService {
    ...
    public findCategoryByClient(id: number, req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<ICategory[]>(`${this.resourceUrl}ByClient/${id}`, { params: options, observe: 'response' });
    }

    public getCategoriesByStatus(id: number, status: boolean, req?: any): Observable<HttpResponse<ICategory[]>> {
        const options = createRequestOption(req);
        return this.http.get<ICategory[]>(`${this.resourceUrl}/getCategoriesByStatus/${id}/${status}`, {
            params: options,
            observe: 'response'
        });
    }
