...
export class DiscountsService {
    ...
    public findDiscountsByClient(id: number, req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<IDiscount[]>(`${this.resourceUrl}ByClient/${id}`, { params: options, observe: 'response' });
    }

    public getDiscountsByStatus(id: number, status: boolean, req?: any): Observable<HttpResponse<IDiscount[]>> {
        const options = createRequestOption(req);
        return this.http.get<IDiscount[]>(`${this.resourceUrl}/getDiscountsByStatus/${id}/${status}`, {
            params: options,
            observe: 'response'
        });
    }
